 ...
 * - 媒体类型：前端直连 row.url（CDN 资源），与聊天附件下载共用 downloadAsset()，
 *   走 fetch → Blob → <a download>，fetch 失败时降级到 anchor 打开新标签。
 *
 *   与聊天附件链路一致：聊天 SSE 资源已经在生产中裸奔 CDN 稳定运行，记忆管理
 *   没必要再走后端 /file/:id/download 代理（保留后端端点作为历史兜底）。
 */
async function downloadFile(row: MemoryFileItem) {
  ...
  try {
    ...
    if (!row.url) {
      ElMessage.warning('该文件没有可下载的链接')
      return
    }
    const result = await downloadAsset(resolveUrl(row.url), row.fileName)
    if (result === 'blob') {
      ElMessage.success(`已开始下载「${row.fileName}」`)
    } else if (result === 'fallback') {
      ElMessage.warning('直连下载失败，已在新窗口打开（请检查 CORS 或网络）')
    }
  } catch (err) {
