 ...
 *  - 编辑：topic 一经填写不可编辑；可新增/删除源文件；可编辑 md（md 在线编辑通过子组件）
 ...
 *
 * 视觉锁：editStatus=1 时禁用 md 在线编辑与"保存"
 */
...
function fileTypeOf(f: File): number {
  if (f.type.startsWith('image/')) return 2
  if (f.type.startsWith('video/')) return 3
  if (f.type.startsWith('audio/')) return 4
  return 1
}
...
async function saveMd() {
  if (!mdKey.value) {
    ElMessage.warning('暂无 md 可保存')
    return
  }
  try {
    await ElMessageBox.confirm('将覆盖原 md 文件，确认保存？', '保存 md', { type: 'warning' })
  } catch {
    return
  }
  mdSaving.value = true
  try {
    const token = await getMemoryUploadToken(memoryId.value, `${memoryId.value}.md`, true)
    // 用 fetch 直接 PUT 到七牛
    const r = await fetch(`${token.domain.startsWith('http') ? '' : '//'}${token.domain}/${token.key}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'text/markdown; charset=utf-8', Authorization: `UpToken ${token.token}` },
      body: mdText.value,
    })
    if (!r.ok) throw new Error(`HTTP ${r.status}`)
    mdDirty.value = false
    ElMessage.success('md 已保存')
  } catch (e) {
    ElMessage.error(e instanceof Error ? e.message : String(e))
  } finally {
  ...
  >
    ...
    <template #footer>
      ...
      <el-button
        ...
        :disabled="mode === 'create' && topicExists"
        @click="handleSubmit"
      >
        {{ mode === 'create' ? '保存' : '保存编辑' }}
      </el-button>
